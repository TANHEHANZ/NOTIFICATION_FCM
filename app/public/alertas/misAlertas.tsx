...
export default function MisAlertas() {
  ...
  const { data } = fetchData("GET /v1/api/alert/myAlert");
  console.log("data", data);
  const handlePressAlert = (alert: any) => {
    console.log("ver alerta primero ", alert.id);
    router.push(`/public/alertas/${alert.id}`);
  };
  ...
  const renderItem = ({ item }: any) => (
    <Card
      ...
      style={{ marginLeft: 10, padding: 0, overflow: "hidden" }}
    >
      <View
